...
func TestShouldUpdate(t *testing.T) {
	tests := map[string]struct {
		...
		"simple secret": {
			secrets: map[dag.Meta]dag.Empty{
				dag.Meta{Name: "test", Namespace: "default"}: {},
			},
		...
		"secret not found": {
			secrets: map[dag.Meta]dag.Empty{
				dag.Meta{Name: "foo", Namespace: "default"}: {},
			},
			obj:  &v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "test", Namespace: "default"}},
			want: false,
		},
		"multiple secrets": {
			secrets: map[dag.Meta]dag.Empty{
				dag.Meta{Name: "foo", Namespace: "default"}:  {},
				dag.Meta{Name: "foo1", Namespace: "default"}: {},
				dag.Meta{Name: "foo2", Namespace: "default"}: {},
				dag.Meta{Name: "foo3", Namespace: "default"}: {},
				dag.Meta{Name: "foo4", Namespace: "default"}: {},
			},
