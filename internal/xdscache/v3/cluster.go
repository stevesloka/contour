...
type ClusterCache struct {
	mu     sync.Mutex
	values map[string]*envoy_v3_cluster.Cluster
	contour.Cond
}

// Update replaces the contents of the cache with the supplied map.
func (c *ClusterCache) Update(v map[string]*envoy_v3_cluster.Cluster) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *ClusterCache) Contents() []proto.Message {
	c.mu.Lock()
	defer c.mu.Unlock()
	var values []*envoy_v3_cluster.Cluster
...
type clusterVisitor struct {
	clusters map[string]*envoy_v3_cluster.Cluster
}

// visitCluster produces a map of *envoy_v3_cluster.Clusters.
func visitClusters(root dag.Vertex) map[string]*envoy_v3_cluster.Cluster {
	cv := clusterVisitor{
		clusters: make(map[string]*envoy_v3_cluster.Cluster),
	}
	cv.visit(root)
	return cv.clusters
}
