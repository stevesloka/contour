...
// NewServiceController creates the service controller from mgr. The controller will be pre-configured
// to watch for Service objects across all namespaces.
func NewServiceController(mgr manager.Manager, eventHandler cache.ResourceEventHandler, log logrus.FieldLogger) (controller.Controller, error) {
	r := &serviceReconciler{
...
func (r *serviceReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if errors.IsNotFound(err) {
		r.Error(nil, "Could not find Service %q in Namespace %q", request.Name, request.Namespace)
